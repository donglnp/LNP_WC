    ...
    <div className="rounded-lg border border-arena-green/40 bg-[radial-gradient(ellipse_at_top,_#0d1f15_0%,_#070A0E_60%)] p-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] tracking-[0.25em] uppercase text-arena-muted">
...
function MatchCard({ match, teams, user, onTeamClick }) {
  ...
  const s = statusOf({ home: h, away: a, locked });
  const ringClass =
    s === "locked"
    ...
    <div className={`rounded-lg border ${ringClass} bg-arena-surface p-4`}>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1">
          <Box value={h} onChange={setH} locked={locked} tone={tone(s)} />
          <Box value={a} onChange={setA} locked={locked} tone={tone(s)} />
        </div>
      ...
      >
        {saving ? t("common.saving") : s === "locked" ? t("matches.edit") : s === "editing" ? t("matches.save") : t("matches.lock_in")}
      </button>
    </div>
  );
}
...
function Side({ team, reverse, onClick }) {
  return (
    <div className={`flex items-center gap-2 ${reverse ? "flex-row-reverse" : ""}`}>
      <FlagBadge team={team} onClick={onClick} />
      <button
        type="button"
        onClick={onClick}
        disabled={!onClick}
...
function Box({ value, onChange, locked, tone }) {
  return (
    <input
      ...
      className={`w-9 h-9 text-center font-mono text-base rounded border bg-arena-card ${tone} outline-none disabled:opacity-100`}
      placeholder="–"
    />
  );
}
...
function StatusBadge({ status }) {
  ...
  if (status === "locked")
    return (
      <span className="text-arena-green inline-flex items-center gap-1">
        <span className="w-1.5 h-1.5 rounded-full bg-arena-green" /> {t("status.locked")}
      </span>
    );
  if (status === "editing")
    return (
      <span className="text-arena-amber inline-flex items-center gap-1">
        <span className="w-1.5 h-1.5 rounded-full bg-arena-amber" /> {t("status.editing")}
      </span>
    );
  return (
    <span className="text-arena-muted inline-flex items-center gap-1">
